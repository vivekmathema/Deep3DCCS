...
- Training: 250 epochs, batch size 8, Adam (lr=1e-4),linear Mean Squared Error (MSE)
- 
...
NOTE: Although a fixed random seed is used to ensure reproducibility, the inherent complexity of the 3D CNN architecture can lead to minor variations between training runs. Nevertheless, the overall performance trend remains consistent, and differences in inference results are not significant. The model usually reaches peak prediction accuracy within 250 epochs, at which point the training was ceased to prioritize robust generalization and avoid overfitting. This approach was supported by the use of the Adam optimizer, which efficiently manages the sparse gradients and high parameter counts of 3DCNN model architecture, and a linear Mean Squared Error (MSE) loss that provides the smooth, distance-based gradients essential for precise regression. Furthermore, a batch size of 8 served as a practical compromise, ensuring gradient stability while staying within the VRAM capacity of modern CUDA-core NVIDIA GPUs. We will also be updating this information on the software repository accordingly. 
...
**Note:mz_ratio data is (optional). However, if the user supplies mz_ratio values, it will be included as a part of information metadata. We highly recommend keeping the mz-ratio data for proper documentation purpose in mass spectrometry**
